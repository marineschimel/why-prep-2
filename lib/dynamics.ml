...
struct
  ...
  let dyn ~readout ~theta ~task =
    ...
    fun ~k ~x ~u ->
      let a = Owl_parameters.extract theta.a in
      let a =
        if Int.(k < n_prep) then AD.Maths.(a / AD.F tau) else AD.Maths.(a / AD.F tau_b)
      in
      let xs = AD.Maths.get_slice [ []; [ 4; -1 ] ] x in
      let thetas = AD.Maths.get_slice [ []; [ 0; 3 ] ] x in
      let xst = AD.Maths.transpose xs in
  ...
  let dyn_x =
    ...
    let _dyn_x ~readout ~theta ~task =
      ...
      fun ~k ~x ~u:_ ->
        let a =
          if Int.(k < n_prep) then AD.Maths.(a / AD.F tau) else AD.Maths.(a / AD.F 150E-3)
        in
...
struct
  ...
  let dyn ~readout ~theta ~task =
    ...
    fun ~k ~x ~u ->
      ...
      in
      let xs = AD.Maths.get_slice [ []; [ 4; -1 ] ] x in
      let n = AD.Mat.col_num xs in
      let us_pre = AD.Maths.get_slice [ []; [ 0; Int.(n / 2) ] ] xs in
      let xs = AD.Maths.get_slice [ []; [ Int.(n / 2); -1 ] ] xs in
      let x_init = task.x0 |> fun z -> AD.Maths.get_slice [ []; [ Int.(n / 2); -1 ] ] z in
      let thetas = AD.Maths.get_slice [ []; [ 0; 3 ] ] x in
      let xst = AD.Maths.transpose xs in
      let s = Arm.pack_state thetas in
      let dx =
        AD.Maths.(
          ((phi_x xs *@ a) - (xs / AD.F tau) + ((u + (bias * F tau_b / F tau)) *@ b))
          * _dt)
      in
      let tau =
        let r = AD.Maths.(phi_x xst) in
      ...
      let new_thetas =
        AD.Maths.(
          of_arrays
            ...
            |])
      and new_x = AD.Maths.(xs + dx) in
      AD.Maths.(concatenate ~axis:1 [| new_thetas; u; new_x |])
    ...
    let _dyn_u ~readout:_ ~theta ~task =
      ...
      fun ~k:_ ~x:_ ~u ->
        let mat =
          AD.Maths.concatenate
            ~axis:0
            [| AD.Mat.zeros 2 m; AD.Mat.zeros 2 m; AD.Maths.((u + bias) *@ b) |]
        in
        AD.Maths.(transpose (mat * dt))
    in
    None
