...
let cost prms =
  ...
  let dyn_prms =
    Dynamics.Arm_Plus_P.
      { a = (learned : setter) AD.Maths.(transpose w)
...
let final_prms =
  let prms0 =
    { u = AD.Mat.gaussian ~sigma:0.1 n n_lr
    ; v = AD.Mat.gaussian ~sigma:0.1 n_lr n
    ; x0 = AD.Mat.gaussian ~sigma:0.1 n_reaches n
    }
  in
  let stop =
    ...
    fun s ->
      ...
      Mat.of_array z (-1) 1
      |> fun x ->
      Mat.save_txt ~append:true ~out:(Printf.sprintf "%s/losses" dir) x;
...
let _ =
  ...
  |> Array.of_list
  |> fun z ->
  Mat.of_array z (-1) 1 |> fun x -> Mat.save_txt ~out:(Printf.sprintf "%s/losses" dir) x
