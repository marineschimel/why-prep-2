...
let data_dir =
  "/home/mmcs3/rds/rds-t2-cs156-T7o4pEA8QoU/mmcs3/random_monkeys_random_lambda_2E-6/ramping/seed_5_200_2"
...
let target =
  Array.init n_reaches (fun i ->
      Mat.load_txt (Printf.sprintf "%s/torques_%i_0" data_dir i)
      |> fun z -> Arr.reshape z [| 1; (Arr.shape z).(0); (Arr.shape z).(1) |])
  |> Arr.concatenate ~axis:0
  |> AD.pack_arr

let placeholder_task x0 =
  Model.
    { t_prep = 0.
    ; x0
    ; t_movs = [| 0.4 |]
...
let integrate ~readout ~prms ~x0 ~task ~n_steps =
  ...
  fun ~n ->
    let us = List.init n_steps (fun _ -> AD.Mat.zeros 1 n) in
    ...
    Array.map
      (fun x ->
        let n_out = AD.Mat.col_num readout in
        let x = AD.Maths.(x *@ readout) in
        AD.Arr.reshape x [| 8; 1; n_out |])
      z
...
let cost prms =
  ...
  let dyn_prms =
    Dynamics.Arm_Plus_P.
      { a = (learned : setter) AD.Maths.(transpose prms.w)
      ; b = (pinned : setter) (AD.Mat.eye n)
      ; bias = (pinned : setter) (AD.Mat.zeros 1 n)
      }
  in
  let traj =
    integrate ~readout:c ~prms:dyn_prms ~x0 ~task:(placeholder_task x0) ~n ~n_steps
  in
  AD.Maths.l2norm_sqr' AD.Maths.(traj - target)

let final_prms =
  let prms0 =
    { w = AD.Mat.gaussian ~sigma:0.1 n n; x0 = AD.Mat.gaussian ~sigma:0.1 n_reaches n }
  in
  let stop =
...
let _ =
  Mat.save_txt ~out:(Printf.sprintf "%s/w" dir) (AD.unpack_arr final_prms.w);
  Mat.save_txt ~out:(Printf.sprintf "%s/x0" dir) (AD.unpack_arr final_prms.x0);
  (* Mat.save_txt ~out:(Printf.sprintf "%s/eigs" dir) (eigenvalues (prms.w)); *)
  Mat.save_txt ~out:(Printf.sprintf "%s/c" dir) (AD.unpack_arr c)

let dyn_prms =
  Dynamics.Arm_Plus_P.
    { a = (learned : setter) AD.Maths.(transpose final_prms.w)
    ; b = (pinned : setter) (AD.Mat.eye n)
    ; bias = (pinned : setter) (AD.Mat.zeros 1 n)
    }
...
let torques =
  integrate
    ...
    ~prms:dyn_prms
    ~x0:final_prms.x0
    ~task:(placeholder_task final_prms.x0)
    ~n
    ~n_steps

let _ =
  let torques i =
    AD.Maths.get_slice [ [ i ] ] torques
    |> AD.unpack_arr
    |> fun x -> Mat.reshape x [| -1; 2 |]
  in
  Array.init n_reaches (fun i ->
      let t = torques i in
      Mat.save_txt ~out:(Printf.sprintf "%s/torques_%i" dir i) t)

let traj =
  integrate
    ~readout:(AD.Mat.eye n)
    ~prms:dyn_prms
    ~x0:final_prms.x0
    ~task:(placeholder_task final_prms.x0)
    ~n
...
let _ =
  let traj i =
    AD.Maths.get_slice [ [ i ] ] traj
    |> AD.unpack_arr
    |> fun x -> Mat.reshape x [| -1; n |]
  in
  Array.init n_reaches (fun i ->
      let t = traj i in
      Mat.save_txt ~out:(Printf.sprintf "%s/rates_%i" dir i) t)
