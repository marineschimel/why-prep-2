...
let x_mov =
  ...
  let f i =
    Arr.reshape
      (Arr.get_slice [ [ i ]; [ t_prep + 50; t_prep + size_mov - 1 + 50 ]; [] ] m)
      [| size_mov; size_net |]
  in
  Array.init n_reaches f |> Arr.concatenate ~axis:0
...
let occupancy_double =
  let load i j =
    let m =
      Mat.load_txt Printf.(sprintf "results/double/soc/compound_%i%i/traj_600" i j)
    in
...
let x =
  Array.init n_reaches (fun i ->
      let y =
        Mat.load_txt (in_dir Printf.(sprintf "reach_%i/traj_%i" (succ i) t_prep))
...
let potency_in_nullspace x =
  ...
  let null_proj = Mat.((eye 200 - (transpose c *@ (inv (c *@ transpose c) *@ c))) *@ x) in
  Mat.(transpose null_proj *@ q *@ null_proj)

let test =
  Mat.map_rows
    potency_in_nullspace
    Mat.(
      get_slice
        [ []; [ 4; -1 ] ]
        (load_txt Printf.(sprintf "double/soc/compound_01/traj_600")))
  |> Mat.concatenate ~axis:0
  |> Mat.save_txt ~out:"test"
