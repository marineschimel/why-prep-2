...
let tangling t traj_1 traj_2 dt =
  ...
  and dx_t =
    Mat.(
      (get_slice [ [ t ]; [] ] traj_1 - get_slice [ [ pred t ]; [] ] traj_1)
      /$ dt)
  and dx_2 =
    Mat.(
      (get_slice [ [ 1; -1 ]; [] ] traj_2 - get_slice [ [ 0; -2 ]; [] ] traj_2)
      /$ dt)
  in
  let entangled =
    Mat.mapi_rows
...
let activity ?reach:_reach time =
  let _reach = match _reach with Some a -> a | None -> 1 in
  let idces =
    List.init (400 / sampling_window) (fun i -> time + (i * sampling_window))
  in
  let x =
    Mat.get_fancy
      [ L idces; R [ 4; -1 ] ]
...
let muscles ?reach:_reach time =
  let _reach = match _reach with Some a -> a | None -> 1 in
  let idces =
    List.init (400 / sampling_window) (fun i -> i * sampling_window)
  in
  let x =
    Mat.get_fancy
      [ L idces; R [ 0; 1 ] ]
    ...
    (fun i ->
      let x =
        Mat.get_slice
          [ [ 400; -200 ]; [ 4; -1 ] ]
          (Mat.load_txt
             (Printf.sprintf "results/weighing_pm/w_%i_%i/reach_%i/traj_400" pc
                pm i))
      in
      let m = Mat.(max ~axis:0 x - min ~axis:0 x +$ 0.) in
      let x = Mat.(x / m) in
      let modes, _, _ =
        Linalg.D.svd (Mat.transpose Mat.(x - Mat.mean ~axis:1 x))
      in
      let pcs = Mat.get_slice [ []; [ 0; dim - 1 ] ] modes in
      let x = Mat.(x *@ pcs) in
      let dx =
        Mat.(get_slice [ [ 1; -1 ]; [] ] x - get_slice [ [ 0; -2 ]; [] ] x)
      in
...
let get_torques_all pc pm =
  Array.map
    (fun i ->
      let x =
        Mat.get_slice [ [ 400; -200 ]; [] ]
          (Mat.load_txt
             (Printf.sprintf "results/weighing_pm/w_%i_%i/reach_%i/torques_400"
                pc pm i))
      in
...
let tangling pc pm =
  ...
  Mat.save_txt
    ~out:
      (Printf.sprintf "results/weighing_pm/entanglement/test_tangling_all_%i_%i"
         pc pm)
      ...
      let x =
        Mat.load_txt
          (Printf.sprintf
             ...
             (int_of_float cp) (int_of_float cm))
      in
...
let _ =
  let m_mov =
    Mat.col
      (Mat.load_txt "results/weighing_pm/entanglement/test_tangling_all_1000_1")
      0
    |> Mat.mean'
  in
  let m_mixed =
    Mat.col
      (Mat.load_txt "results/weighing_pm/entanglement/test_tangling_all_1_1")
      0
    |> Mat.mean'
  in
  let m_prep =
    Mat.col
      (Mat.load_txt "results/weighing_pm/entanglement/test_tangling_all_1_1000")
      0
    |> Mat.mean'
  in
  Mat.save_txt ~out:"results/weighing_pm/entanglement/comp_entanglement"
