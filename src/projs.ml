...
let softmax x =  let m = Mat.(max ~axis:0 x - min ~axis:0 x +$ (0.2 *. max' x)) in
  Mat.(x / m)
...
let duration t = let rates0 =  Mat.load_txt (in_dir Printf.(sprintf "rates_0_%i" t)) in Mat.row_num rates0 
...
let preprocessed_data t =
  ...
  in
  mean_data |> fun a ->  Arr.reshape a [| (Arr.shape a).(0); (Arr.shape a).(1) |], Mat.of_arrays (Arr.to_arrays dat)
...
let projs reach =
  Array.map
    ~f:(fun t ->
      let m = let m = Arr.reshape (snd (preprocessed_data t)) [| n_reaches; duration t ; -1 |] in
        Arr.reshape
          (Arr.get_slice [ [ reach ]; []; [] ] m)
          [| -1; 200 |]
      in
      let top_pc_mov, top_pc_prep =
        Mat.get_slice [ []; [ 0; 2 ] ] top_pc_mov, Mat.get_slice [ []; [ 0; 2 ] ] top_pc_prep in 
      Mat.(m *@ (top_pc_mov))
