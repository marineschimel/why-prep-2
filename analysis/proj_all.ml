...
let data_mov =
    let load i t_prep =  let m = Mat.load_txt (Printf.(sprintf "reach_1/reach_%i/traj_%i" i t_prep)) in
    let ma = Mat.get_fancy [ R [ t_prep; t_prep + 400 -1 ]; R [ 4; -1 ] ] m in
    Arr.reshape ma [| 400 ; size_net; 1 |]
in let get_data t_prep = 
...
in (Array.map (fun t_prep -> get_data t_prep) times) |> Mat.concatenate ~axis:0
...
let projs = Array.map (fun t -> let m = Mat.load_txt (Printf.(sprintf "reach_1/reach_1/traj_%i" t)) |> Mat.get_slice [[0;duration t];[4;-1]] in
let top_pc_mov, top_pc_prep = Mat.get_slice [[];[0;1]] top_pc_mov, Mat.get_slice [[];[2]] top_pc_prep in 
Mat.(m*@(top_pc_mov@||top_pc_prep)) |> Mat.save_txt ~out:(in_dir (Printf.sprintf "proj_all_%i" t))) times