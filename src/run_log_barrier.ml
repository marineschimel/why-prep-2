...
let save_results suffix xs us =
  ...
  Owl.Mat.save_txt ~out:(file "rates") (link_f xs);
  Owl.Mat.save_txt ~out:(file "eff_us") (link_f us);
  Owl.Mat.save_txt ~out:(file "torques") Mat.(xs *@ transpose (AD.unpack_arr c))
...
(* (struct
let phi_u x = AD.Maths.(F 0.5 * (x + sqrt (sqr x + F 4.)))
  let d_phi_u x = AD.Maths.((F 0.5 * (F 1. + (x / sqrt (sqr x + F 4.)))))
  let d2_phi_u x = AD.Maths.(F 2. / ((sqr x + F 4.) * sqrt (sqr x + F 4.)))
end) *)
(*   let phi_x x = AD.Maths.(F 0.5 * (x + sqrt (sqr x + F 4.)))
  let d_phi_x x = AD.Maths.((F 0.5 * (F 1. + (x / sqrt (sqr x + F 4.)))))
  let d2_phi_x x = AD.Maths.(F 2. / ((sqr x + F 4.) * sqrt (sqr x + F 4.))) *)
(* module D0 = Dynamics.Arm_Plus (struct
  let phi_x x = AD.Maths.(F 0.5 * (x + sqrt (sqr x + F 4.)))
  let d_phi_x x = AD.Maths.(diagm (F 0.5 * (signum x + F 1.)))
  let phi_u x = AD.Maths.(F 0.5 * (x + sqrt (sqr x + F 4.)))
...
module L0 = Likelihoods.End_Phi (struct
  let label = "output"
  let phi_x x = AD.Maths.(relu x)
  let d_phi_x x = AD.Maths.(F 0.5 * (signum x + F 1.))
  let d2_phi_x _ = AD.Mat.zeros 1 m
end)
...
let _ =
  let x0 =
    AD.Maths.concatenate ~axis:1 [| theta0; AD.Maths.(F 1E-5 * AD.Mat.ones 1 m) |]
  in
  ...
  Array.mapi tasks ~f:(fun i t ->
      ...
      then (
        ...
        let xs, us, _ =
          I.solve ~u_init:Mat.(1E-5 $* Mat.ones 2001 m) ~n:(m + 4) ~m ~x0 ~prms t
        in
