...
let proj i =
  let b = Mat.load_txt (in_dir (Printf.sprintf "b_new_%i" i)) in
  let c = Mat.load_txt (in_dir (Printf.sprintf "c_new_%i" i)) in
  let _proj = Mat.(c *@ transpose b) in
  let cg = ctrl_gramian b in
  let co = obs_gramian c in
  Mat.(trace cg), Mat.trace co

(* let evol_projs = Array.init 15 (fun i -> (proj (succ i)) |> Mat.sqr |> Mat.sum ~axis:1 |> Mat.transpose) |> Mat.concatenate ~axis:0 *)

let evol_traces =
  Array.init 16 (fun i ->
      let cg, co = proj (succ i) in
      [| cg; co |])
  |> Mat.of_arrays
