...
let rinv = Cmdargs.(get_int "-rinv" |> force ~usage:"-rinv [inverse of r_coeff]")
let noises = [| 0.000; 0.001; 0.010; 0.100; 1.000; 2.000; 10.000 |]
let tprep = 300

let thetas_in, traj_in =
  let x = Mat.load_txt (Printf.sprintf "results/noise/soc_r_%i/traj_%i" rinv tprep) in
  ...
  Array.map
    (fun n ->
      ...
      Mat.(
        ...
        @|| Mat.(
              load_txt
                (Printf.sprintf "results/noise/soc_r_%i/level_%.3f/loss_time" rinv n))))
    noises
    ...
    (fun n ->
      let traj =
        Mat.get_slice
          [ []; [ 4; -1 ] ]
          (Mat.load_txt
             (Printf.sprintf "results/noise/soc_r_%i/level_%.3f/traj_%i" rinv n tprep))
      in
