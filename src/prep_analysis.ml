...
let wm, wp, dat =
  ...
  let prms = P.prms sf in
  let _, wp, wm = cost prms in
  AD.unpack_arr wm, AD.unpack_arr wp, snd preprocessed_data

let proj_wm =
  let m = Arr.reshape dat [| n_reaches * duration; -1 |] in
  let proj_m = Mat.(m *@ wm) in
  let proj_m = Mat.(proj_m - mean ~axis:0 proj_m) in
  let cov_proj = Mat.(transpose proj_m *@ proj_m) in
  let cm, _, _ = Linalg.D.svd cov_proj in
  let proj_p = Mat.(m *@ wp) in
  let proj_p = Mat.(proj_p - mean ~axis:0 proj_p) in
  let cov_proj = Mat.(transpose proj_p *@ proj_p) in
  let cp, _, _ = Linalg.D.svd cov_proj in
  Array.init n_reaches ~f:(fun j ->
      let proj_p_i =
        Mat.(
          get_slice
            [ [ Int.(j * duration); Int.(((j + 1) * duration) - 1) ] ]
            Mat.(proj_p *@ cp))
      in
      let proj_m_i =
        Mat.(
      ...
      Mat.save_txt
        proj_p_i
        ~out:(in_dir (Printf.sprintf "analysis_%i/proj_p_pc_%i" n_dim j));
      Mat.save_txt
        proj_m_i
        ~out:(in_dir (Printf.sprintf "analysis_%i/proj_m_pc_%i" n_dim j)))

(* let proj_wm = 
let proj_wp =  
then do PCA on the projected activity and get top PC*)
