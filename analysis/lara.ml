...
let preprocessed_data =
  let load i =
    let m = Mat.load_txt (in_dir Printf.(sprintf "reach_%i/traj_%i" i t_prep)) in
...
let _ =
  Mat.save_txt ~out:"x_prep" x_prep;
  Mat.save_txt ~out:"x" (snd preprocessed_data)
...
let x_mov =
  ...
  let f i =
    Arr.reshape
      (Arr.get_slice [ [ i ]; [ t_prep; t_prep + size_mov - 1 ]; [] ] m)
      [| size_mov; size_net |]
  in
  Array.init n_reaches f |> Arr.concatenate ~axis:0
...
let _ = Mat.save_txt ~out:(in_dir "analysis/cp") corr_p, Mat.save_txt ~out:(in_dir "analysis/cm") corr_m
...
let modes_jpca x =
  let diff x = Mat.((get_slice [[0;-2];[]] x) - (get_slice [[1;-1];[]] x)) in 
...
let occupancy_double =
  let load i j=
    let m = Mat.load_txt (Printf.(sprintf "double/soc/compound_%i%i/traj_600" i j)) in
  ...
  let rps =
    Arr.concatenate
      ...
      (Array.map (fun (i,j) ->
           ...
           let m = Arr.reshape rp [| (Arr.shape rp).(0); (Arr.shape rp).(1); 1 |] in
           m) [|(0,1);(0,3);(0,4);(0,5);(0,6);(1,2);(1,3);(1,5);(2,3);(3,2);(3,4);(4,3);(5,6);(6,0);(6,5)|])
  and rms =
    Arr.concatenate
      ~axis:2
      (Array.map (fun (i,j) ->
  ...
  let vprep, vmov =
    ...
    , let x = Arr.var ~axis:2 rms |> Arr.sum ~axis:1 |> Arr.to_array in
      Mat.of_array x (-1) 1 )
  in
  ( Mat.save_txt ~out:("double/analysis/vprep") Mat.(vprep /$ max' vprep)
  , Mat.save_txt ~out:("double/analysis/vmov") Mat.(vmov /$ max' vmov) )
...
let proj_pc =
  ...
  fun i ->
    ...
    Mat.save_txt ~out:(in_dir (Printf.sprintf "analysis/proj_pc_prep_%i" i)) proj_prep;
    Mat.save_txt ~out:(in_dir (Printf.sprintf "analysis/proj_pc_all_%i" i)) Mat.(proj_prep@||proj_mov);
    Mat.save_txt ~out:(in_dir (Printf.sprintf "analysis/proj_pc_mov_%i" i)) proj_mov;
    Mat.save_txt ~out:(in_dir (Printf.sprintf "analysis/proj_ort_prep_%i" i)) proj_prep_ort;
    Mat.save_txt ~out:(in_dir (Printf.sprintf "analysis/proj_ort_mov_%i" i)) proj_mov_ort
...
let potency_in_nullspace x = let x = Mat.(transpose (x/$(l2norm' x +. 1E-8))) in let w = Mat.load_txt (in_dir "w") in let c = Mat.load_txt (in_dir "c") 
    in let a = Mat.((w - eye 200)/$Defaults.tau) in let q = obs_gramian a c 
  in let null_proj = Mat.((eye 200 - (transpose c)*@(inv (c*@(transpose c))*@c))*@x)
in Mat.((transpose null_proj)*@q*@(null_proj))

let test = (Mat.map_rows (potency_in_nullspace) ( Mat.(get_slice [[];[4;-1]] (load_txt
(Printf.(sprintf "double/soc/compound_01/traj_600")))))) |> Mat.concatenate ~axis:0 |> Mat.save_txt ~out:"test"
(*
let ()  =   
