...
let obs_id_proj =
  Array.init n_reaches ~f:(fun i ->
      let t = Mat.(traj i - mean_trajs) in
      Mat.(t *@ obs_id_modes) |> fun z -> Arr.reshape z [| 1; -1; 200 |])
  |> fun z ->
  Arr.concatenate ~axis:0 z
  |> fun e -> Arr.std ~axis:0 e |> fun e -> Mat.reshape e [| -1; 200 |]

let obs_c_proj =
  Array.init n_reaches ~f:(fun i ->
      let t = Mat.(traj i - mean_trajs) in
      Mat.(t *@ obs_c_modes) |> fun z -> Arr.reshape z [| 1; -1; 200 |])
  |> fun z ->
  Arr.concatenate ~axis:0 z
  |> fun e -> Arr.std ~axis:0 e |> fun e -> Mat.reshape e [| -1; 200 |]
...
let _ =
  Mat.save_txt (in_dir "obs_id_proj") obs_id_proj;
