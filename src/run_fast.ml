...
let beta = AD.F 1E-2
let phi_x x = x
let d_phi_x _x = AD.Mat.ones 1 m
let d2_phi_x _x = AD.Mat.zeros 1 m
...
let _ =
  let x0 = AD.Maths.concatenate ~axis:1 [|theta0;AD.Maths.(F (-0.0001)*AD.Mat.ones 1 m)|] in 
  ...
  Array.mapi tasks ~f:(fun i t ->
      ...
      then (
        ...
        let xs, us,_  = I.solve ~u_init:(Mat.zeros 2001 m) ~n:(m+4) ~m 
        ~x0 ~prms t in
