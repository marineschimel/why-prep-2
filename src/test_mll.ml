...
let w_skew seed =let m = Mat.gaussian ~sigma:Float.(5. /. sqrt (of_int m)) m m in Mat.((m - transpose m)/$2.)

let w_nonnorm seed = let m = Mat.gaussian ~sigma:Float.(rad_w /. sqrt (of_int m)) m m in Mat.triu ~k:1 m
...
let parameters =
  let open Owl_parameters in
    C.broadcast' (fun () -> Array.init Int.((Array.length nets)*(Array.length lambdas)*(Array.length seeds)) 
    ~f:(
      fun i -> 
        ...
        let n_net = Int.(n_rest / (Array.length lambdas))
        in let n_lambda = Int.rem n_rest n_lambdas
    in 
    let c = (c seed) in 
    let x0_base = (AD.Maths.get_slice [[];[4;-1]] x0)|> AD.Maths.transpose in 
    let u0 = phi_x x0_base  in 
    let norm_u0 = AD.Maths.(l2norm_sqr' u0) in 
    let _ = Stdio.printf "%i %i %i %i%!" (AD.Mat.row_num c) (AD.Mat.col_num c) (AD.Mat.row_num u0) (AD.Mat.col_num u0) in 
    let c = AD.Maths.(c - (c *@ u0 *@ transpose u0 / norm_u0)) in 
    let net_name = nets.(n_net) in 
    let w = if net_name == "soc" then w_soc seed
    else if net_name == "skew" then w_skew seed
    else if net_name == "nonnorm" then w_nonnorm seed
    else w_rdn seed in 
    let baseline_input = AD.Maths.(neg ((AD.pack_arr w *@ link_f x0_base) - x0_base)) |> AD.Maths.transpose in 
...
let sample_from_q ~mu ~q_cov_chol ~n_samples =
  ...
  let _ = Stdio.printf "%i %i %i %i %!" (AD.shape mu).(0) (AD.shape mu).(1) (AD.Mat.row_num (q_cov_chol).(0)) 
  (AD.Mat.row_num (q_cov_chol).(1)) in
...
let q_cov_chol ?(epsilon = 0.) inv_q_covs = 
    ...
    |> Array.map ~f:(fun x ->
            ...
            let y = AD.Maths.(AD.Linalg.inv x + F epsilon * AD.Mat.eye n) in AD.Linalg.chol y)
...
let log_p ~prms ~task us = 
  let _ = Stdio.printf "%i %i %i %!" (Arr.shape us).(1) (Arr.shape us).(2) (Arr.shape us).(0) in 
...
AD.unpack_flt (f k x)) ~n_prep (Mat.reshape us [|(Arr.shape us).(1); (Arr.shape us).(2)|])
in l |> AD.pack_flt
...
let logq us mu_u q_cov_chol =
  let us = AD.pack_arr us in 
    ...
    let du = AD.Maths.(us - (AD.Arr.reshape mu_u [|1; (AD.shape mu_u).(0); (AD.shape mu_u).(1)|])) in
    (* quadratic term: 
...
in let v = Array.foldi u_samples ~init:[] ~f:(fun i acc u -> 
    ...
    let llik = 
      log_lik_term ~prms ~readout:(Owl_parameters.extract prms.Full_P.readout.c) ~task (AD.pack_arr u) in
...
let get_mll xs us quus n_target n_prep task prms =
  ...
  let mll = eval_ml ~epsilon:0. ~mu_u:us ~q_cov_chol ~n_samples:1000 ~task ~prms in AD.unpack_flt mll


let approx_posteriors =
  let x0 = x0 in
  let v = Array.foldi parameters ~init:[] ~f:(fun i acc (prms, net_name, scale_prep, scale_mov, seed) ->
...
in (prms, net_name, scale_prep, scale_mov, seed, xs, us, quus)::acc) else acc) |> C.gather
      in 
      C.broadcast' (fun () -> let _ = Stdio.printf "Broadcasting approx posteriors %!" in v |> Array.to_list |> List.concat) 
...
let get_mll = 
  ...
  C.root_perform (fun () -> Mat.save_txt ~out:(in_dir (Printf.sprintf "mll_%s_%.1f_%.1f_%i" net_name scale_prep scale_mov seed)) (Mat.of_arrays [|[|mll|]|]);
  save_prms  (Printf.sprintf "%s_%.1f_%.1f_%i" net_name scale_prep scale_mov seed) prms;
  save_task (Printf.sprintf "%i_%i" n_target Int.(2*n_prep)) t);
  compute_mll tl
  |[] -> Stdio.printf "done %!"
  in compute_mll approx_posteriors