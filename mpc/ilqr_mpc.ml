...
let run
  ...
  let rec plan_traj step traj inputs x0 us =
    ...
    then
      ...
      let stop =
        ...
        fun k us ->
          ...
          then
            Printf.printf
              "loop %i | iter %i | cost %f | pct change %f\n%!"
              step
              k
              c
              pct_change;
      ...
      in
      let planned_traj = AD.unpack_arr (AD.Maths.get_slice [ []; [ 4; -1 ] ] plan_x) in
      let _ =
        if step = 1
        then (
          ...
          Mat.save_txt ~append:true ~out:(PT.saving_dir "planned") (AD.unpack_arr plan_x);
        Mat.save_txt
          ~append:true
          ~out:(PT.saving_dir "plan_torques")
          Mat.(planned_traj *@ transpose c)
      ...
      in
      let next_u0 =
        if Maths.abs (float (step_go - step)) < float n_planned
        then List.init n_horizon (fun _ -> AD.Mat.zeros 1 size_inputs)
        else
    ...
    then (
      ...
      Mat.save_txt
        ~out:(PT.saving_dir "torques")
        Mat.(AD.unpack_arr (AD.Maths.get_slice [ []; [ 4; -1 ] ] traj) *@ transpose c);
      ...
      in
      let next_u0 =
        Array.sub (us |> Array.of_list) n_planned (n_horizon - n_planned) |> Array.to_list
      in
      plan_traj
        (step + n_planned)
        (plan_x :: traj)
