...
struct
  ...
  let start ~task t = let t_prep = AD.F task.t_prep in AD.Maths.(sigmoid ((t_prep - t)/(F 2E-4)))

  let finish ~task t = let t_mov = AD.F (task.t_prep +. task.t_mov) in AD.Maths.(sigmoid ((t-t_mov)/(F 20E-3)))
  ...
  let neg_logp_t ~prms ~task =
    ...
    fun ~k ~z_t ->
      ...
      let mu_t = AD.Maths.(z_t *@ c_t) in
        AD.Maths.(
          start ~task t * ((F 0.5 * sum' (t_coeff * sqr mu_t))
          + (F 0.5 * qs_coeff * l2norm_sqr' (thetas - theta0)))
          + finish ~task t * (F 0.5 * g_coeff * (l2norm_sqr' (theta_pos - target_pos) + (F 0.1 * l2norm_sqr' theta_vel))))
  ...
  let neg_jac_t =
    let _neg_jac_t ~prms ~task =
      ...
      fun ~k ~z_t ->
        ...
        let x = AD.Maths.get_slice [ []; [ 4; -1 ] ] z_t in
        let r_state =
          AD.Maths.(start ~task t * (t_coeff * x *@ c_t *@ c))
        in
        let r_xp = AD.Maths.(start ~task t * (qs_coeff * (theta_pos - theta0)) + 
          finish ~task t * (g_coeff * (theta_pos - target_pos)))
        in
        let r_xv =
          AD.Maths.(start ~task t * (qs_coeff * theta_vel) + finish ~task t * (F 0.1 * g_coeff * theta_vel))
        in
        AD.Maths.(concatenate ~axis:1 [| r_xp; r_xv; r_state |])
    in
    Some _neg_jac_t
  ...
  let neg_hess_t =
    let _neg_hess_t ~prms ~task =
      ...
      fun ~k ~z_t:_ ->
        let t = AD.F Float.(of_int k *. dt) in 
        let mu = AD.Maths.(start ~task t * (qs_coeff * AD.Mat.eye 2)
          + finish ~task t * (g_coeff * AD.Mat.eye 2))
        in
        let mv =
           AD.Maths.(start ~task t * (qs_coeff * AD.Mat.eye 2) + 
