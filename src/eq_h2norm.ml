...
let dir =
  Cmdargs.(
    get_string "-d"
    |> default "/home/mmcs3/rds/hpc-work/_results/why_prep/results/uniform_1E-7/skew")

let data_dir =
  Cmdargs.(
    get_string "-data" |> default "/home/mmcs3/rds/hpc-work/_results/why_prep/data")
...
let w0 =
  let m = AD.Mat.gaussian ~sigma:(0.6 /. sqrt 200.) n n in
  AD.Maths.(m - transpose m)

let h2norm w c =
  let a = AD.Maths.(w - AD.Mat.eye n) in
  let p = AD.Linalg.lyapunov (AD.Maths.transpose a) AD.Maths.(neg (AD.Mat.eye n)) in
  AD.Maths.trace AD.Maths.(c *@ p *@ transpose c)
...
let cost prms =
  ...
  let h2norm = h2norm w c0 in
  AD.Maths.(sqr (h2_tgt - h2norm))
...
let learn alpha0 =
  ...
  let alpha = prms.alpha |> AD.unpack_flt in
  let mu = prms.mu |> AD.unpack_flt in
  let _ = Printf.printf "alpha = %f || mu %f || val : %f %!" alpha mu (P.fv sf) in
  alpha, mu

let _ =
  let alpha, mu = learn 0. in
  let new_w = AD.Maths.((w0 * AD.F alpha) + (AD.F mu * AD.Mat.eye n)) |> AD.unpack_arr in
  let eigenvalues m =
  ...
  in
  Mat.save_txt ~out:(in_dir "test_w") new_w;
