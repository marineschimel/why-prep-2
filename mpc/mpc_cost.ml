...
module C_Early (P : Prms) = struct
  ...
  let t_left =
    ...
    if float ds > 0. then float ds *. sampling_dt
  ...
  let cost ~u ~x ~k =
    let _ =
      if k mod 1000 = 0 then Printf.printf "T left %f step %i \n %!" t_left step
    in
  ...
  let rl_x =
    let rlx ~k ~x ~u:_u =
      ...
      let r_xv1 =
        AD.Maths.(dx_vel *@ q * sigmoid ((t - tau) / F 20E-3) * F 0.2)
      in
      let r_xv2 = AD.Maths.(dx_vel *@ q_start * start * F 2.) in
      AD.Maths.(
        concatenate ~axis:1 [| r_xp1 + r_xp2; r_xv1 + r_xv2; r_xstate |] * __dt)
    in
  ...
  let rl_xx =
    let rlxx ~k ~x:_x ~u:_u =
      ...
      let mu =
        AD.Maths.(
          AD.Mat.eye 2 * AD.F Defaults.q_coeff * __dt
          * sigmoid ((t - tau) / F 20E-3)
          + (AD.F P.qs_coeff * AD.Mat.eye 2 * start * __dt))
      in
      let mv =
        AD.Maths.(
          __dt * AD.Mat.eye 2 * F Defaults.q_coeff
      ...
      let mf1 =
        AD.Maths.concatenate ~axis:1
          [| mu; AD.Mat.zeros (n_theta - 2) (size_net + 2) |]
      in
      let mf2 =
        AD.Maths.concatenate ~axis:1
          [|
            ...
            mv;
            AD.Mat.zeros (n_theta - 2) size_net;
          |]
      in
      let mf3 =
        AD.Maths.concatenate ~axis:1 [| AD.Mat.zeros size_net n_theta; mx |]
      in
  ...
  let final_cost ~x ~k:_k =
    ...
    let fl =
      ...
      let dx_p = AD.Maths.(tgt_pos - thetas) and dx_vel = thetas_dot in
      AD.(
        Maths.(
          (sum' (dx_p *@ q * dx_p) * F 0.) + (F 0. * sum' (dx_vel *@ q * dx_vel))))
    in
    fl
...
module C_Post (P : Prms) = struct
  ...
  let rl_x =
    let _rlx ~k ~x ~u:_u =
      ...
      in
      AD.Maths.(
        concatenate ~axis:1 [| r_xp1; r_xv1; AD.Mat.zeros 1 P.m |] * __dt)
    in
  ...
  let rl_xx =
    let _rlxx ~k ~x:_x ~u:_u =
      ...
      let mf3 =
        AD.Maths.concatenate ~axis:1
          [| AD.Mat.zeros size_net (size_net + n_theta) |]
      in
      AD.Maths.concatenate ~axis:0 [| mf1; mf2; mf3 |]
    in
    None
