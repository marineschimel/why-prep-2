...
let ctr_modes = C.broadcast' (fun () ->
  ...
  let ctr_modes, _,_ = Linalg.D.svd ctr in ctr_modes
  )
...
let all_prms =
  C.broadcast' (fun () -> Array.init 100 ~f:(fun i ->
    let c = Mat.get_slice [[]; [2*i; 2*i +1]] ctr_modes 
  in let c = Mat.(c /$ l2norm' c)
in let c = Mat.(c *$ Float.(rad_c *. 0.05)) in 
