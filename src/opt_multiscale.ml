...
(* let init_prms = Misc.read_bin (in_dir "progress_2001.params.bin") *)
...
let loss ~u_init ~prms t =
  ...
  ( AD.Maths.(
      (l + (F 0.01 * l2norm_sqr' c) + (F 1E-5 * l2norm_sqr' a))
      / F (Float.of_int n_samples))
  , AD.unpack_arr us )
...
let final_prms =
  ...
  I.train
    ...
    ~loss
    ~recycle_u:true
    ~eta:(`of_iter (fun k -> Float.(0.01 / sqrt (of_int k /. 20.))))
    ~init_prms
    ~in_each_iteration
