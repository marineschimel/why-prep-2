...
(* let _ =
  C.root_perform (fun () ->
      Mat.save_txt ~out:(Printf.sprintf "%s/c" dir) (Mat.gaussian ~sigma:0.003 2 m)) *)
...
let x0 =
  C.broadcast' (fun () ->
      let x = AD.pack_arr Mat.(load_txt (Printf.sprintf "%s/x0_opt" dir)) in
      AD.Maths.(x / F 5.))
...
let x0 = C.broadcast' (fun () -> AD.pack_arr (Mat.load_txt (Printf.sprintf "%s/x0" dir)))
