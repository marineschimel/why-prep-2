...
let accuracy_theta x =
  ...
  let _ = Mat.print fp in
  let _ = Mat.print tp in
...
let dir =
  Cmdargs.(get_string "-d" |> force ~usage:"-dp [dir where param search is saved]")

let ts = [| 0.; 0.1; 0.5; 1.; 2.5; 5. |]
let rs = [| 1.; 1.2; 1.5; 2.; 3.; 5.; 7.; 10.; 100.; 1000. |]

let torques_cost t_prep x =
  Mat.(l2norm_sqr' (get_slice [ [ 0; int_of_float (t_prep *. 1000.) ]; [] ] x))

let load_traj r t = Mat.load_txt (Printf.sprintf "%s/r_%.3f/t_%.3f/traj_300" dir r t)

let load_torques r t =
  Mat.load_txt (Printf.sprintf "%s/r_%.3f/t_%.3f/torques_300" dir r t)

let _ = Printf.printf "checkpoint 1 %!"

let load_us r t =
  let _ = "true tr" in
  Mat.load_txt (Printf.sprintf "%s/r_%.3f/t_%.3f/results_us_300" dir r t)

let load_linesearch r t =
  Mat.load_txt (Printf.sprintf "%s/r_%.3f/t_%.3f/linesearch" dir r t)
    ...
    (fun r ->
      Array.map
        (fun t ->
          ...
          let inpts = Mat.get loss_time (n - 1) 3 in
          let tot_loss = Mat.get loss_time (n - 1) 2 in
          let torques = Mat.get loss_time (n - 1) 4 in
          let acc = Mat.get loss_time (n - 1) 5 in
          ...
          Mat.of_arrays
            [| [| 0.1 *. r /. float size_net; t; acc; torques; inpts; tot_loss; torques |]
            |])
        ts
      |> fun y -> Mat.concatenate ~axis:0 y)
    rs
