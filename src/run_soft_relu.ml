...
let b = Mat.concatenate ~axis:0 [|Mat.concatenate ~axis:1 [|Mat.eye 120; Mat.zeros 120 80|]; Mat.concatenate ~axis:1 [|Mat.(neg (eye 80)); Mat.zeros 80 120|]|]
...
let _ =
  let x0 = AD.Maths.concatenate ~axis:1 [|theta0;AD.Maths.(F (-1.)*AD.Mat.ones 1 m)|] in 
  ...
  Array.mapi tasks ~f:(fun i t ->
      ...
      then (
        ...
        let xs, us,_  = I.solve ~u_init:(Mat.(gaussian ~sigma:0.01 2001 m)) ~n:(m+4) ~m 
        ~x0 ~prms t in
