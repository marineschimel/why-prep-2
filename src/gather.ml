...
let _ =
  Array.map
    (fun psi ->
      ...
      let w = let x = 
  Mat.gaussian size_net size_net ~sigma:(psi/.(sqrt (float size_net))) 
      in Mat.(x - (transpose x)) in 
...
let _ =
  Array.map
    (fun psi ->
    let w =
      let x = Mat.gaussian size_net size_net ~sigma:(psi/.(sqrt (float size_net))) 
          in Mat.(x - (transpose x)) in 
      let a = Mat.((w - eye size_net)/$tau) in 
      let _, eigs = Linalg.D.eig a in
      let im, re = Z.im eigs, Z.re eigs in
