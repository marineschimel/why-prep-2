...
let tasks =
  Array.init (n_targets * 40) ~f:(fun i ->
      let n_time = 0 in
      let x0 = AD.Mat.gaussian ~sigma:Float.(of_int i /. 40.) 2 1 in
      let x0 =
        AD.Maths.concatenate [| x0; AD.Maths.transpose theta0 |] ~axis:0
        |> AD.Maths.transpose
      in
...
let () =
  ...
  Array.iteri tasks ~f:(fun i (_, t) ->
      ...
      then (
        ...
        in
        let () =
          save_results (Printf.sprintf "%i" i) xs us quus n_target n_prep t;
          ...
          let w =
            ...
            |> AD.unpack_arr
          in
          projs w (AD.unpack_arr xs) (Printf.sprintf "x_%i" i);
          projs w (AD.unpack_arr us) (Printf.sprintf "u_%i" i)
        in
        if save_all
        then (
