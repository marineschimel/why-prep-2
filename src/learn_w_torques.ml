...
module Prms = struct
  type 'a t = { x0 : 'a; w : 'a } [@@deriving prms]
end
...
let cost prms = 
  let x0 = prms.x0 in 
  let dyn_prms =  Dynamics.Arm_Plus_P.{ a = (learned : setter) (AD.Maths.(transpose prms.w))
...
let final_prms =
  let prms0 = { w = AD.Mat.gaussian ~sigma:0.1 n n; x0 =  AD.Mat.gaussian ~sigma:0.1 n_reaches n} in
let stop =
...
let dyn_prms =  Dynamics.Arm_Plus_P.{ a = (learned : setter) (AD.Maths.(transpose final_prms.w))
  ; b = (pinned : setter) ((AD.Mat.eye n))
  ; bias = (pinned : setter) ((AD.Mat.zeros 1 n))
  }
