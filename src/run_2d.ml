...
let rads = [|0.0;0.1;0.4;0.5;0.8;1.0;1.5;2.0;2.5;3.0;4.0;5.0;6.0;7.0;8.0;10.0|]
...
let () =
  ...
  let _ = save_prms "" prms in
    Array.iteri prms ~f:(fun i (rad, prms) ->
      let n_target = 0 in let _, t = tasks.(0) in
      ...
      in 
          ...
          save_results
            ...
            projs w (AD.unpack_arr xs) (Printf.sprintf "x_%i" t_prep_int); projs w (AD.unpack_arr us)  (Printf.sprintf "u_%i" t_prep_int)
        in
        if save_all
        then (
