...
module Make (P : Prms) (D : C_Prms) = struct
  ...
  let ms ~x =
    ...
    let m12 =
      ...
      and mat2 =
        ...
        |> of_arrays
      and mat1 =
        [| [| AD.F 0.; (F 2. * dotdot1) + dotdot2 |]; [| F 0.; dotdot1 |] |]
        |> of_arrays
      in
      (cst2 * mat2) + (cst1 * mat1)
    in
  ...
  let dyn ~k ~x ~u =
    ...
    let s = Arm.pack_state thetas in
    let __a x =
      AD.Maths.(((AD.pack_arr P.w *@ g x) - (AD.Mat.eye size_net *@ x)) / F tau)
    in
    let t = AD.Maths.(F (float_of_int k) * __dt) in
    let _switch = if t < AD.F P.t_prep then AD.F 0. else AD.F 1. in
    let _input_switched =
  ...
  let _lin_dyn_x ~k ~x ~u:_u =
    ...
    let m21, m22 = ms ~x in
    let __ap x =
      AD.Maths.(((AD.pack_arr P.w *@ diff_g x) - AD.Mat.eye size_net) / F tau)
    in
    (*((Defaults.__w *@ diff_g x) - AD.Mat.eye P.m)*)
    let b1 =
      AD.Maths.concatenate ~axis:1
        [| AD.Mat.zeros 2 2; AD.Mat.eye 2; AD.Mat.zeros 2 size_net |]
    and b2 =
      AD.Maths.(
        concatenate ~axis:1
          [|
    ...
    and b3 =
      AD.Maths.concatenate ~axis:1
        [| AD.Mat.zeros size_net 2; AD.Mat.zeros size_net 2; __ap x |]
    in
    let mat =
      AD.Maths.(concatenate ~axis:0 [| b1 * _switch; _switch * b2; b3 |])
    in
  ...
  let _lin_dyn_u ~k ~x:_x ~u:_u =
    ...
    let m =
      AD.Maths.concatenate ~axis:0
        [|
          AD.Mat.zeros 2 m; AD.Mat.zeros 2 m; AD.Maths.(P.b * _input_switched);
        |]
    in
...
module Make_4D (P : Prms) (D : C_Prms) = struct
  ...
  let ms ~x =
    ...
    let s = Arm.pack_state thetas in
    let tau =
      AD.Maths.(cmc *@ phi (__c *@ g (transpose xs))) |> AD.Maths.transpose
    in
    let dotdot = M.theta_dot_dot s tau in
    let dotdot1, dotdot2 = (AD.Mat.get dotdot 0 0, AD.Mat.get dotdot 1 0) in
    let m12 =
      ...
      AD.Maths.(
        concatenate ~axis:1
          [|
            ...
            neg
              AD.Maths.(
                nminv *@ (cmc *@ dphi (__c *@ transpose x_state)) *@ __c);
          |])
    and b3 =
      AD.Maths.concatenate ~axis:1
        [| AD.Mat.zeros size_net 2; AD.Mat.zeros size_net 2; __ap |]
    in
    let mat =
      AD.Maths.(concatenate ~axis:0 [| b1 * _switch; _switch * b2; b3 |])
    in
